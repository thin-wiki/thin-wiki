...
$(document).ready(function(){
    $("#saveGithubStorage").click(function(){
        ...
        if (actionType === 'new') {
            reqType = "POST";
            reqUrl = "/api/storage/github";
        } else {
            reqType = "PUT";
            reqUrl = "/api/storage/github/" + id;
        }
    ...
    $('.deleteStorage').click(function(){
        ...
        layer.confirm('您确定要删除['+title+']存储么？', {
            ...
            $.ajax({
                type: "DELETE",
                url: "/api/storage/github/"+storageId,
                contentType: "application/json",
                dataType: "json",
                success: function (data) {
                    if (data.code !== 0){
...
$('#editModal').on('show.bs.modal', function (event) {
    ...
    if (actionType === 'edit'){
        ...
        modal.find('.modal-title').text('新增Github存储')
